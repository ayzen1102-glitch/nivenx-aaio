...
class DisconnectMemberSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        try:
            ...
            parts = []
            if disconnected:
                parts.append(f"{E.SUCCESS} Disconnected: {', '.join(disconnected)}")
            if failed:
                parts.append(f"{E.ERROR} Failed: {', '.join(failed)}")

            msg = "\n".join(parts) or f"{E.ERROR} No members were disconnected."
            await interaction.response.edit_message(
                content=None,
                view=make_container(f"-# {msg}")
            )
        except Exception as exc:
            logger.error("VM", f"Disconnect callback error: {exc}", exc)
            await interaction.response.edit_message(
                content=None,
                view=make_container(f"-# {E.ERROR} An error occurred.", accent=COLOR_ERROR)
            )
...
INTERFACE_TITLE = "## Join2Create | Interface"

INTERFACE_COMMANDS = (
    f"{E.LOCK} — [`Lock`]({SUPPORT_URL}) the voice channel\n"
    f"{E.UNLOCK} — [`Unlock`]({SUPPORT_URL}) the voice channel\n"
    f"{E.GHOST} — [`Hide`]({SUPPORT_URL}) the voice channel\n"
    f"{E.REVEAL} — [`Reveal`]({SUPPORT_URL}) the voice channel\n"
    f"{E.CLAIM} — [`Claim`]({SUPPORT_URL}) the voice channel\n"
    f"{E.DISCONNECT} — [`Disconnect`]({SUPPORT_URL}) a member\n"
    f"{E.INFO} — [`View`]({SUPPORT_URL}) channel information\n"
    f"{E.INCREASE} — [`Increase`]({SUPPORT_URL}) the user limit\n"
...
class VoiceMasterInterface(discord.ui.LayoutView):
    def __init__(self):
        ...
        btn_lock       = discord.ui.Button(emoji=E.LOCK,       style=discord.ButtonStyle.gray, custom_id="vm_lock")
        btn_unlock     = discord.ui.Button(emoji=E.UNLOCK,     style=discord.ButtonStyle.gray, custom_id="vm_unlock")
        ...
        btn_claim      = discord.ui.Button(emoji=E.CLAIM,      style=discord.ButtonStyle.gray, custom_id="vm_claim")
        ...
        btn_increase   = discord.ui.Button(emoji=E.INCREASE,   style=discord.ButtonStyle.gray, custom_id="vm_increase")
    ...
    async def _on_disconnect(self, interaction: discord.Interaction):
        ...
        if not members:
            await interaction.response.send_message(
                view=make_container(f"-# {E.ERROR} No members to disconnect.", accent=COLOR_ERROR),
                ephemeral=True
            )
            return
        await interaction.response.send_message(
    ...
    async def _resolve(self, interaction: discord.Interaction, *, owner_required: bool = True):
        if not interaction.user.voice or not interaction.user.voice.channel:
            await interaction.response.send_message(
                view=make_container(f"-# {E.ERROR} You must be in a voice channel.", accent=COLOR_ERROR),
                ephemeral=True
            )
        ...
        if not cog:
            await interaction.response.send_message(
                view=make_container(f"-# {E.ERROR} VoiceMaster is not available.", accent=COLOR_ERROR),
                ephemeral=True
            )
            return None, None, True
        ...
        if owner_required and not await cog.is_channel_owner(interaction.user.id, channel.id):
            await interaction.response.send_message(
                view=make_container(f"-# {E.ERROR} You don't own this voice channel.", accent=COLOR_ERROR),
                ephemeral=True
            )
            return None, None, True
    ...
    async def _dispatch(self, interaction: discord.Interaction, action: str):
        ...
        try:
            if action == "lock":
                ...
                await interaction.response.send_message(
                    view=make_container(f"-# {E.LOCK} Voice channel locked.", accent=COLOR_SUCCESS), ephemeral=True
                )
            elif action == "unlock":
                await cog.unlock_channel(channel, interaction.user)
                await interaction.response.send_message(
                    view=make_container(f"-# {E.UNLOCK} Voice channel unlocked.", accent=COLOR_SUCCESS), ephemeral=True
                )
            elif action == "ghost":
                ...
                if await cog.claim_channel(channel, interaction.user):
                    await interaction.response.send_message(
                        view=make_container(f"-# {E.CLAIM} Voice channel claimed.", accent=COLOR_SUCCESS), ephemeral=True
                    )
                else:
                    await interaction.response.send_message(
                        view=make_container(f"-# {E.ERROR} Cannot claim this channel.", accent=COLOR_ERROR), ephemeral=True
                    )
            elif action == "info":
                await interaction.response.send_message(
                ...
                await interaction.response.send_message(
                    view=make_container(f"-# {E.INCREASE} User limit increased.", accent=COLOR_SUCCESS), ephemeral=True
                )
            ...
            logger.error("VM", f"Interface action '{action}' error: {exc}", exc)
            await interaction.response.send_message(
                view=make_container(f"-# {E.ERROR} An error occurred.", accent=COLOR_ERROR), ephemeral=True
            )
...
class VoiceMaster(commands.Cog):
    ...
    async def _create_temp_channel(self, member: discord.Member, category: discord.CategoryChannel = None):
        try:
            ...
            logger.error("VM", f"Create temp channel error: {exc}", exc)
            return None
    ...
    async def _cleanup_empty_channel(self, channel: discord.VoiceChannel):
        try:
            ...
            logger.error("VM", f"Cleanup channel {channel.id} error: {exc}", exc)
    ...
    async def claim_channel(self, channel: discord.VoiceChannel, member: discord.Member) -> bool:
        try:
            ...
            logger.error("VM", f"Claim channel error: {exc}", exc)
            return False
    ...
    async def on_voice_state_update(self, member, before, after):
        try:
            ...
            logger.error("VM", f"Voice state update error: {exc}", exc)
    ...
    @checks.cooldown()
    async def _join2create(self, ctx):
        if not ctx.invoked_subcommand:
            ...
            container.add_item(ui.TextDisplay(
                ...
                "**Getting Started**\n"
                f"-# Use `join2create setup` to configure VoiceMaster for your server\n\n"
                "**Popular Commands**\n"
                f"-# `join2create lock` — Lock your channel\n"
                f"-# `join2create claim` — Claim an inactive channel\n"
                f"-# `join2create limit <number>` — Set user limit"
            ))
            container.add_item(ui.Separator())
    ...
    @_join2create.command(brief="Configure VoiceMaster for your server")
    @checks.has_perms(manage_guild=True)
    async def setup(self, ctx):
        try:
            category     = await ctx.guild.create_category("AeroX | Join2Create")
            ...
            container.add_item(ui.TextDisplay(f"## {E.SUCCESS} VoiceMaster Setup Complete"))
            container.add_item(ui.Separator())
            container.add_item(ui.TextDisplay(
                f"**Join Channel:** {join_channel.mention}\n"
                f"**Category:** {category.name}\n\n"
                f"-# Use `join2create sendinterface` to send the control panel."
            ))
            view.add_item(container)
            await ctx.send(view=view, allowed_mentions=discord.AllowedMentions.none())

        except Exception as exc:
            logger.error("VM", f"Setup error: {exc}", exc)
            await ctx.fail("Failed to set up VoiceMaster. Please check my permissions.")

    @_join2create.command(brief="Send the VoiceMaster control panel")
    @checks.has_perms(manage_guild=True)
    async def sendinterface(self, ctx):
        await ctx.send(view=VoiceMasterInterface())

    @_join2create.command(brief="Reset VoiceMaster configuration for this server")
    @checks.has_perms(manage_guild=True)
    async def reset(self, ctx):
        try:
            ...
            logger.error("VM", f"Reset error: {exc}", exc)
            await ctx.fail("Failed to reset VoiceMaster.")

    @_join2create.command(brief="Set the category for temporary voice channels")
    @checks.has_perms(manage_guild=True)
    async def category(self, ctx, cat: discord.CategoryChannel = None):
        try:
            ...
            self._bust_cache(ctx.guild.id)
            msg = f"{E.CATEGORY} Voice channels will be created in: **{cat.name}**" if cat \
                else f"{E.CATEGORY} Voice channels will be created without a specific category."
            await ctx.success(msg)
        except Exception as exc:
            logger.error("VM", f"Category error: {exc}", exc)
            await ctx.fail("Failed to set category.")

    @_join2create.group(brief="Configure default settings for new voice channels")
    @checks.has_perms(manage_guild=True)
    async def default(self, ctx):
        if not ctx.invoked_subcommand:
            settings = await self.get_guild_settings(ctx.guild.id)
            if not settings:
                await ctx.fail("VoiceMaster is not set up. Use `join2create setup` first.")
                return
            ...
            container.add_item(ui.Separator())
            container.add_item(ui.TextDisplay(
                f"**{E.RENAME} Default Name:** {default_name}\n"
    ...
    async def default_name(self, ctx, *, name: str):
        ...
        try:
            ...
            self._bust_cache(ctx.guild.id)
            await ctx.success(f"{E.RENAME} Default channel name set to: **{name}**")
        except Exception as exc:
            logger.error("VM", f"Default name error: {exc}", exc)
    ...
    async def default_limit(self, ctx, limit: int):
        ...
        try:
            ...
            logger.error("VM", f"Default limit error: {exc}", exc)
    ...
    async def default_bitrate(self, ctx, bitrate: int):
        ...
        try:
            ...
            logger.error("VM", f"Default bitrate error: {exc}", exc)
    ...
    async def default_region(self, ctx, region: str = None):
        ...
        try:
            ...
            logger.error("VM", f"Default region error: {exc}", exc)
            await ctx.fail("Failed to set default region.")

    @_join2create.command(brief="View current voice channel configuration")
    ...
    @_join2create.command(brief="Lock your voice channel")
    async def lock(self, ctx):
        ...
        try:
            ...
            await ctx.success(f"{E.LOCK} Voice channel locked.")
        except Exception as exc:
            logger.error("VM", f"Lock error: {exc}", exc)
            await ctx.fail("Failed to lock the voice channel.")

    @_join2create.command(brief="Unlock your voice channel")
    async def unlock(self, ctx):
        ...
        try:
            ...
            await ctx.success(f"{E.UNLOCK} Voice channel unlocked.")
        except Exception as exc:
            logger.error("VM", f"Unlock error: {exc}", exc)
            await ctx.fail("Failed to unlock the voice channel.")

    @_join2create.command(brief="Hide your voice channel")
    async def ghost(self, ctx):
        ...
        try:
            ...
            logger.error("VM", f"Ghost error: {exc}", exc)
            await ctx.fail("Failed to hide the voice channel.")

    @_join2create.command(brief="Unhide your voice channel")
    async def unghost(self, ctx):
        ...
        try:
            ...
            logger.error("VM", f"Unghost error: {exc}", exc)
            await ctx.fail("Failed to reveal the voice channel.")

    @_join2create.command(brief="Claim an inactive voice channel")
    async def claim(self, ctx):
        if not ctx.author.voice or not ctx.author.voice.channel:
            await ctx.fail("You must be in a voice channel to use this command.")
            return
        if await self.claim_channel(ctx.author.voice.channel, ctx.author):
            await ctx.success(f"{E.CLAIM} Voice channel claimed.")
        else:
            await ctx.fail("Cannot claim this channel. The owner may still be present.")

    @_join2create.command(brief="Set a user limit on your voice channel")
    async def limit(self, ctx, limit: int):
        ...
        try:
            ...
            logger.error("VM", f"Limit error: {exc}", exc)
            await ctx.fail("Failed to set user limit.")

    @_join2create.command(brief="Rename your voice channel")
    async def name(self, ctx, *, name: str):
        ...
        try:
            ...
            await ctx.success(f"{E.RENAME} Channel renamed to **{name}**.")
        except Exception as exc:
            logger.error("VM", f"Rename error: {exc}", exc)
            await ctx.fail("Failed to rename the voice channel.")

    @_join2create.command(brief="Set bitrate of your voice channel")
    async def bitrate(self, ctx, bitrate: int):
        ...
        try:
            ...
            logger.error("VM", f"Bitrate error: {exc}", exc)
            await ctx.fail("Failed to set bitrate.")

    @_join2create.command(brief="Transfer ownership of your channel")
    async def transfer(self, ctx, member: converters.DiscordMember):
        ...
        try:
            ...
            await ctx.success(f"{E.OWNER} Ownership transferred to **{member.display_name}**.")
        except Exception as exc:
            logger.error("VM", f"Transfer error: {exc}", exc)
            await ctx.fail("Failed to transfer ownership.")

    @_join2create.command(brief="Allow a member or role to join your VC")
    async def permit(self, ctx, target: Union[converters.DiscordMember, discord.Role]):
        ...
        try:
            ...
            await ctx.success(f"{E.SUCCESS} **{target.name}** can now join your voice channel.")
        except Exception as exc:
            logger.error("VM", f"Permit error: {exc}", exc)
            await ctx.fail("Failed to permit access.")

    @_join2create.command(brief="Block a member or role from joining your VC")
    async def reject(self, ctx, target: Union[converters.DiscordMember, discord.Role]):
        ...
        try:
            ...
            await ctx.success(f"{E.ERROR} **{target.name}** can no longer join your voice channel.")
        except Exception as exc:
            logger.error("VM", f"Reject error: {exc}", exc)
            await ctx.fail("Failed to reject access.")

    @_join2create.command(brief="Set a status message for your voice channel")
    async def status(self, ctx, *, status: str = None):
        ...
        try:
            ...
            logger.error("VM", f"Status error: {exc}", exc)
            await ctx.fail("Failed to set channel status.")

    @_join2create.command(brief="Disconnect a member from your voice channel", hidden=True)
    async def disconnect(self, ctx, member: converters.DiscordMember):
        ...
        try:
            ...
            logger.error("VM", f"Disconnect error: {exc}", exc)
            await ctx.fail("Failed to disconnect the member.")

    @_join2create.command(brief="Configure a role for your voice channel members")
