...
module.exports = {
    name: "ammultiwhitelist",
    aliases: ["ammultiwl", "ammwl"],
    description: "Add or remove multiple users from automod whitelist at once",
    category: "automod",
    cooldown: 3,
    ...
    run: async (client, message, args, prefix) => {
        ...
        const key       = `amwhitelist_${message.guild.id}`;
            ...
            return message.reply({
                components: [
                    new ContainerBuilder()
                        .setAccentColor(0x26272F)
                        .addTextDisplayComponents(
                            new TextDisplayBuilder().setContent("## Automod Multi Whitelist")
                        )
                        .addSeparatorComponents(sep())
                        .addTextDisplayComponents(
                            new TextDisplayBuilder().setContent(
                                `\`${prefix}ammultiwhitelist add <@users...>\` — Add multiple users\n` +
                                `\`${prefix}ammultiwhitelist remove <@users...>\` — Remove multiple users`
                            )
                        )
                        .addSeparatorComponents(sep())
                        .addTextDisplayComponents(
        ...
        if (opt === "add") {
            ...
            for (const user of users) {
                if (!client._amWhitelistCache.has(message.guild.id)) client._amWhitelistCache.set(message.guild.id, new Set());
                client._amWhitelistCache.get(message.guild.id).add(user.id);
            }
        ...
        if (opt === "remove") {
            ...
            if (!client._amWhitelistCache) client._amWhitelistCache = new Map();
            for (const user of users) {
                if (client._amWhitelistCache.has(message.guild.id)) {
                    client._amWhitelistCache.get(message.guild.id).delete(user.id);
                }
            }
